...
echo "🚀 Starting Parrot OS flash process (flash-sd_v1.3)..."
echo "Image:    $IMG"
echo "Device:   $DEVICE"
...
if [ "$NUM_PARTS" -eq 0 ]; then
    ...
    # Mount with read-write to allow SSH file creation
    if ! sudo mount -o rw "$BOOT_DEV" /mnt/flash-sd-boot; then
        echo "⚠️ Boot partition mounted read-only, attempting remount rw..."
        sudo mount -o remount,rw "$BOOT_DEV" /mnt/flash-sd-boot || echo "⚠️ Could not mount boot partition read-write."
    fi
    BOOT_MOUNT="/mnt/flash-sd-boot"

    echo "Mounting root: $ROOT_DEV -> /mnt/flash-sd-root"
    if ! sudo mount "$ROOT_DEV" /mnt/flash-sd-root; then
        echo "⚠️ Could not mount root partition, skipping root customization."
    else
        ROOT_MOUNT="/mnt/flash-sd-root"
    fi
fi
